...
import 'package:her_driver_app/data/error_response.dart';
...
class ApiClient extends GetxService {
  ...
  Future<Response> getZone(String type) async {
    try {
      if(kDebugMode) {
        print('====> API Call: /api/v1/driver/zone\nHeader: $_mainHeaders');
      }
      http.Response response = await http.post(
        Uri.parse('$appBaseUrl/api/v1/driver/zone'),
        body: jsonEncode({'type': type}),
        headers: _mainHeaders,
      ).timeout(Duration(seconds: timeoutInSeconds));
      return handleResponse(response, '/api/v1/driver/zone');
    } catch (e) {
      return Response(statusCode: 1, statusText: noInternetMessage);
    }
  }
