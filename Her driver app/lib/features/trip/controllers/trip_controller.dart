...
import 'package:her_driver_app/features/trip/domain/models/trip_cancellation_cause_list_model.dart';
...
import 'package:her_driver_app/features/trip/domain/models/trip_model.dart';
import 'package:her_driver_app/features/trip/domain/models/trip_overview_model.dart';
...
class TripController extends GetxController implements GetxService{
  ...
  Future<void> verifyOtp(String tripId, String otp) async {
    isLoading = true;
    update();
    Response response = await tripServiceInterface.verifyOtp(tripId, otp);
    if (response.statusCode == 200) {
      showCustomSnackBar('OTP verified successfully.', isError: false);
      // Call startTrip endpoint
      Response startTripResponse = await tripServiceInterface.startTrip(tripId);
      if (startTripResponse.statusCode == 200) {
        Get.find<RideController>().getRideDetails(tripId).then((_) {
          Get.offAll(() => const DashboardScreen());
        });
      } else {
        ApiChecker.checkApi(startTripResponse);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  void navigateToOtpScreen(String tripId) {
    Get.to(() => OtpEntryScreen(tripId: tripId));
  }

  void checkSpecialRideTypes(TripDetail tripDetail) {
    if (tripDetail.trip?.withMaleCompanion == true) {
      showCustomSnackBar('This ride includes a male companion.', isError: false);
    }
    if (tripDetail.trip?.isChildFriendly == true) {
      showCustomSnackBar('This is a child-friendly ride.', isError: false);
    }
    if (tripDetail.trip?.isNannyRide == true) {
      showCustomSnackBar('This is a nanny ride.', isError: false);
    }
    if (tripDetail.trip?.babySeatCount > 0) {
      showCustomSnackBar('This ride requires ${tripDetail.trip?.babySeatCount} baby seat(s).', isError: false);
    }
  }
}
