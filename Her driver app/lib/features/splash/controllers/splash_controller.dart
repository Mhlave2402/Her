...
class SplashController extends GetxController implements GetxService {
  ...
  Future<void> getZone() async {
    Position? position = await Get.find<LocationController>().getCurrentLocation();
    if (position != null) {
      Response response = await splashServiceInterface.getZone(position.latitude.toString(), position.longitude.toString());
      if (response.statusCode == 200) {
        Get.find<AuthController>().setCountryCode(response.body['data']['country_dial_code']);
      } else {
        ApiChecker.checkApi(response);
      }
    }
