...
import 'package:her_driver_app/features/profile/domain/models/level_model.dart';
...
import 'package:her_driver_app/features/profile/domain/models/reward_model.dart';
...
import 'package:her_driver_app/common_widgets/config.dart';
...
>>>>>>>
  ...
  Future<Response> updateProfile( String firstName, String lastName, String email, String identityNumber, List<String> services, bool isNanny, bool isKidsOnlyVerified, bool isChildFriendly, bool acceptsLowerTierRides, bool hasBabySeat, bool acceptsMaleCompanion) async {
    isLoading = true;
    update();
  ...
  Future<Response> profileOnlineOffline( bool value) async {
    ...
    if(response!.statusCode == 200){
      if(isOnline == "0"){
        isOnline = "1";
        Get.find<BehaviorController>().startMonitoring();
      }else if(
      isOnline == "1"){
        isOnline = "0";
        Get.find<BehaviorController>().stopMonitoring();
      }
    }else{
      Get.back();
      isLoading = false;
 ...
 Future<void> getProfileLevelInfo() async{
  ...
  Future<void> getTierSettings() async {
    final response = await Get.find<DriverPrefsRepository>().getTierSettings();
    if (response != null) {
      tierList = List<String>.from(response['tiers']);
      registeredTier = response['driver']['registered_tier'];
      globalPrefs = Map<String, bool>.from(response['preferences']['global']);
      perVehiclePrefs = Map<String, Map<String, bool>>.from(
          response['preferences']['per_vehicle']);
      vehicles = response['vehicles'];
      update();
    }
  }

  void updateGlobalTierPreference(String tier, bool value) {
    globalPrefs[tier] = value;
    update();
    Get.find<DriverPrefsRepository>().updateTierSettings(globalPrefs, perVehiclePrefs);
  }

  void updatePerVehicleTierPreference(String vehicleId, String tier, bool value) {
    if (perVehiclePrefs.containsKey(vehicleId)) {
      perVehiclePrefs[vehicleId]![tier] = value;
    } else {
      perVehiclePrefs[vehicleId] = {tier: value};
    }
    update();
  ...
  void toggleVehicleActive(String vehicleId) {
    Get.find<DriverPrefsRepository>().toggleVehicleActive(vehicleId);
    getTierSettings();
  }
}
