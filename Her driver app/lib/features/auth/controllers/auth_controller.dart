...
import 'package:her_driver_app/features/auth/domain/enums/verification_from_enum.dart';
...
import 'package:her_driver_app/features/splash/domain/models/config_model.dart';
...
import 'package:her_driver_app/features/auth/screens/sign_in_screen.dart';
import 'package:her_driver_app/features/auth/widgets/approve_dialog_widget.dart';
...
import 'package:her_driver_app/features/auth/screens/verification_screen.dart';
import 'package:her_driver_app/features/location/screens/access_location_screen.dart';
...
import 'package:her_driver_app/common_widgets/snackbar_widget.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  FocusNode identityNode = FocusNode();
  ...
  Future<void> register(String code, SignUpBody signUpBody) async {
    ...
    signUpBody.identificationType = _identityType;
    signUpBody.identityNumber = identityController.text;
  ...
  _navigateLogin(String code,String phone, String password) async {
    if (_isActiveRememberMe) {
      saveUserCredential(code ,phone, password);
    } else {
      clearUserCredential();
    }
    Get.find<ProfileController>().getProfileInfo().then((value) async {
      if(value.statusCode == 200){
        if(await Get.find<AuthController>().getZoneId() == ''){
  ...
  Future<bool> isLoggedIn() async {
    return await authServiceInterface.isLoggedIn();
  }
  ...
  Future<String> getUserNumber() async {
    return await authServiceInterface.getUserNumber();
  }

  Future<String> getUserCountryCode() async {
    return await authServiceInterface.getUserCountryCode();
  }

  Future<String> getLoginCountryCode() async {
    return await authServiceInterface.getLoginCountryCode();
  }

  Future<String> getUserPassword() async {
    return await authServiceInterface.getUserPassword();
  }

  Future<bool> isNotificationActive() async {
    return await authServiceInterface.isNotificationActive();
  }

  toggleNotificationSound() async {
    authServiceInterface.toggleNotificationSound(!await isNotificationActive());
    update();
  }
  ...
  Future<String> getUserToken() async {
    return await authServiceInterface.getUserToken();
  }

  Future<String> getDeviceToken() async {
    return await authServiceInterface.getDeviceToken();
  }

  Future <void> setUserToken(String token) async{
    authServiceInterface.saveUserToken(token, await getZoneId());
  }
  ...
  Future<String> getZoneId() async {
    return await authServiceInterface.getZonId();
  }
