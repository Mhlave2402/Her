...
import 'package:her_driver_app/features/auth/screens/otp_log_in_screen.dart';
import 'package:her_driver_app/features/auth/screens/sign_up_screen.dart';
import 'package:her_driver_app/features/html/domain/html_enum_types.dart';
...
import 'package:her_driver_app/features/auth/screens/forgot_password_screen.dart';
import 'package:her_driver_app/features/html/screens/policy_viewer_screen.dart';
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> _initializeCountryCode() async {
    String? countryCode = Get.find<AuthController>().getLoginCountryCode();
    if (countryCode.isNotEmpty) {
      Get.find<AuthController>().countryDialCode = countryCode;
    } else {
                        ...
                        ButtonWidget(
                          ...
                          onPressed: () async {
                            ...
                            if (canAuthenticate) {
                              ...
                              if (didAuthenticate) {
                                String phone = await Get.find<AuthController>().getUserNumber();
                                String password = await Get.find<AuthController>().getUserPassword();
                                String countryCode = await Get.find<AuthController>().getLoginCountryCode();
                                if(phone.isNotEmpty && password.isNotEmpty){
                                  authController.login(countryCode,phone, password);
                                }else{
                                  showCustomSnackBar('phone_and_password_are_required'.tr);
                                }
                              }
                            }
                          }, radius: 50,
