...
class TripRequestService extends BaseService implements TripRequestServiceInterface
{
    ...
    public function index(array $criteria = [], array $relations = [], array $whereHasRelations = [], array $orderBy = [], int $limit = null, int $offset = null, array $withCountQuery = [], array $appends = [], array $groupBy = []): Collection|LengthAwarePaginator
    {
        $data = [];
        $whereInCriteria = [];
        $searchData = []; // Initialize searchData at the start
        $whereBetweenCriteria = []; // Initialize whereBetweenCriteria at the start
        ...
        return $this->tripRequestRepository->getBy(
            criteria: $data, 
    ...
    public function findNearestDrivers(string $latitude, string $longitude, string $zoneId, int|string $radius, string $vehicleCategoryId = null, string $requestType = null, string $rideRequestType = null, int|string $parcelWeight = null, bool $with_male_companion = false, string $preferred_gender = null, bool $childFriendly = false): mixed
    {
        $attributes = [
            'latitude' => $latitude,
            'longitude' => $longitude,
        ...
        if ($preferred_gender != 'any') {
            $attributes['gender'] = $preferred_gender;
        }

        if ($childFriendly) {
            $attributes['is_child_friendly'] = $childFriendly;
        }
        ...
        if ($driverList->isEmpty() && $vehicleCategoryId) {
            $category = \Modules\VehicleManagement\Entities\VehicleCategory::find($vehicleCategoryId);
            if ($category) {
                $rank = $category->rank;
                $higherRankCategories = \Modules\VehicleManagement\Entities\VehicleCategory::where('rank', '>', $rank)->pluck('id');
                if ($higherRankCategories->isNotEmpty()) {
                    $attributes['vehicle_category_id'] = $higherRankCategories->toArray();
                    $attributes['accepts_lower_tier_rides'] = true;
                    $driverList = $this->userLastLocationRepository->getNearestDrivers($attributes);
                }
            }
        }
        ...
        return $driverList->filter(function ($driver) use ($requestType, $rideRequestType, $maxParcelRequestAcceptLimitStatus, $maxParcelRequestAcceptLimitCount) {
            ...
            if ($requestType === RIDE_REQUEST && $rideRequestType === 'regular' && $destinationCoordinates && isset($destinationCoordinates['destination_coordinates']['coordinates'])) {
                $data['from_longitude'] = (float)$driver->longitude;
                $data['from_latitude'] = (float)$driver->latitude;
    ...
    public function createRideRequest(array $attributes = []): mixed
    {
        ...
        try {
            $customer = $this->userRepository->findOne(id: $attributes['customer_id']);
            $activeShortfall = $this->tripRequestRepository->findOneBy(criteria: [
                'customer_id' => $customer->id,
                'is_shortfall_active' => true,
            ]);

            if ($activeShortfall) {
                $attributes['actual_fare'] += $activeShortfall->shortfall_per_trip;
                $activeShortfall->shortfall_installments_remaining -= 1;
                $activeShortfall->shortfall_paid_total += $activeShortfall->shortfall_per_trip;

                if ($activeShortfall->shortfall_installments_remaining <= 0) {
                    $activeShortfall->is_shortfall_active = false;
                    $activeShortfall->shortfall_recovery_completed = true;
                }
                $activeShortfall->save();
            }

            $baby_seat_fee = 0;
            $child_friendly_fee = 0;
            if ($attributes['baby_seat']) {
                $fee = BabySeatFee::first();
                if ($fee) {
                    $baby_seat_fee = $fee->fee;
                }
            }

            if ($attributes['child_friendly']) {
                $child_friendly_fee = config('child_friendly.fee');
            }
            $trip = $this->tripRequestRepository->create([
                ...
                'with_male_companion' => $attributes['with_male_companion'] ?? false,
                'baby_seat' => $attributes['baby_seat'] ?? false,
                'child_friendly' => $attributes['child_friendly'] ?? false,
            ]);
            $trip->tripStatus()->create([
                'customer_id' => $attributes['customer_id'],
                'pending' => now()
            ...
            $trip->fee()->create([
                'baby_seat_fee' => $baby_seat_fee,
                'child_friendly_fee' => $child_friendly_fee
            ]);
    ...
    public function storeScreenShot(array $attributes): mixed
    {
        ...
        if ($attributes['file'] && $trip) {
            $data = [
                'map_screenshot' => fileUploader('trip/screenshots/', 'png', $attributes['file'], $trip->map_screenshot),
            ];
    ...
    public function handleShortfall(array $attributes): mixed
    {
        $trip = $this->tripRequestRepository->findOne(id: $attributes['trip_request_id']);
        if ($trip) {
            $shortfall_amount = $attributes['shortfall_amount'];
            $shortfall_per_trip = round($shortfall_amount / 3, 2);
            $trip->update([
                'shortfall_amount' => $shortfall_amount,
                'amount_paid_in_cash' => $attributes['amount_paid_in_cash'] ?? 0,
                'is_shortfall_active' => true,
                'shortfall_installments_remaining' => 3,
                'shortfall_per_trip' => $shortfall_per_trip,
                'shortfall_recovery_completed' => false,
                'shortfall_paid_total' => 0,
            ]);
        }
        return $trip;
    }

    public function updateTrip(array $attributes, string $id): mixed
    {
        return $this->tripRequestRepository->update($id, $attributes);
    }

    public function processCashPayment(array $attributes): mixed
    {
        $trip = $this->tripRequestRepository->findOne(id: $attributes['trip_request_id']);
        ...
        $paymentDifference = $attributes['amount_paid_in_cash'] - $trip->actual_fare;

        if ($paymentDifference > 0) {
            // Overpayment
            $customer = $trip->customer;
            $customer->wallet_balance += $paymentDifference;
            $customer->save();

            return ['status' => 'overpaid', 'amount' => $paymentDifference];

        } elseif ($paymentDifference < 0) {
            // Shortfall
            $shortfallAmount = abs($paymentDifference);

            $shortfallAttributes = [
                'trip_request_id' => $attributes['trip_request_id'],
                'shortfall_amount' => $shortfallAmount,
                'amount_paid_in_cash' => $attributes['amount_paid_in_cash'],
            ];

            $updatedTrip = $this->handleShortfall($shortfallAttributes);

            $customer = $updatedTrip->customer;
            $customer->trust_score -= 10;
            $customer->save();

            return ['status' => 'shortfall', 'data' => $updatedTrip];
        }
