...
class UserLastLocationRepository extends BaseRepository implements UserLastLocationRepositoryInterface
{
    ...
    public function getNearestDrivers($attributes): mixed
    {
        return $this->lastLocation
            ...
            ->when(array_key_exists('gender', $attributes), function ($query) use ($attributes) {
                $query->whereHas('user', fn($q) => $q->where('gender', $attributes['gender']));
            })
            ->when(array_key_exists('is_child_friendly', $attributes), function ($query) use ($attributes) {
                $query->whereHas('driverDetails', fn($q) => $q->where('is_child_friendly', $attributes['is_child_friendly']));
            })
