...
class VehicleController extends BaseController
{
    ...
    public function store(VehicleStoreUpdateRequest $request): RedirectResponse
    {
        ...
        if ($request->hasFile('vehicle_photos')) {
            ...
            foreach ($request->file('vehicle_photos') as $photo) {
                $photoPaths[] = fileUploader('vehicle/', 'png', $photo);
            }
            $data['vehicle_photos'] = $photoPaths;
        }

        if ($request->hasFile('documents')) {
            $documentPaths = [];
            foreach ($request->file('documents') as $document) {
                $documentPaths[] = fileUploader('vehicle/document/', 'pdf', $document);
            }
            $data['documents'] = $documentPaths;
        }

        $data['vehicle_request_status'] = APPROVED;
    ...
    public function update(VehicleStoreUpdateRequest $request, string $id): RedirectResponse
    {
        ...
        $this->vehicleService->updatedByAdmin(id: $id, data: $data);
