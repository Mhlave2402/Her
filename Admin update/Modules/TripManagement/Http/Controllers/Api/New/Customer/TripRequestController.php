...
use Modules\FareManagement\Service\Interface\SurgePricingServiceInterface;
        ...
        try
        {
            ...
            if ($trip->driver_id && $request->status == 'cancelled' && $trip->current_status == ONGOING && $trip->type == PARCEL) {
                ...
                if ($parcelReturnTimeFeeStatus) {
                    if ($timeType === 'hour') {
                        $returningAttributes['return_time'] = Carbon::now()->addHours($time);
                    } else {
                        $returningAttributes['return_time'] = Carbon::now()->addDays($time);
                    }
                }
