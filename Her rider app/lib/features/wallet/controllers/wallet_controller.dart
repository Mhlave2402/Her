...
import 'package:her_user_app/features/wallet/domain/models/loyalty_point_model.dart';
import 'package:her_user_app/features/wallet/domain/models/transaction_model.dart';
...
class WalletController extends GetxController implements GetxService{
  ...
  Future<void> transferWalletMoney(String balance) async{
    ...
    if(response.statusCode == 200){
      ...
      Get.find<ProfileController>().getProfileInfo();
      Get.back();
      customSnackBar('transfer_successfully'.tr,isError: false);
    }else{
      isLoading = false;
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> redeemGiftCard(String code) async {
    isLoading = true;
    update();
    Response response = await walletService.redeemGiftCard(code);
    if (response.statusCode == 200) {
      isLoading = false;
      getTransactionList(1);
      Get.find<ProfileController>().getProfileInfo();
      Get.back();
      customSnackBar('gift_card_redeemed_successfully'.tr, isError: false);
    } else {
      isLoading = false;
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> topUpWithCard(String amount, String token) async {
    isLoading = true;
    update();
    Response response = await walletService.topUpWithCard(amount, token);
    if (response.statusCode == 200) {
      isLoading = false;
      getTransactionList(1);
      Get.find<ProfileController>().getProfileInfo();
      Get.back();
      customSnackBar('top_up_successful'.tr, isError: false);
    } else {
      isLoading = false;
      ApiChecker.checkApi(response);
    }
