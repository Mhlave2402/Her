              ...
              Container(
                ...
                ButtonWidget(buttonText: 'convert_point'.tr, width: 145,
                  ...
                  onPressed: (){
                    ...
                    if(point.isEmpty) {
                      customSnackBar('please_input_point'.tr,);
                    }else if(double.parse(point)< (Get.find<ConfigController>().config?.conversionRate ?? 0)) {
                      customSnackBar('${'minimum_conversion_point'.tr}: '
                          '${Get.find<ConfigController>().config?.conversionRate!}',
                      );
                    }else{
                      if(walletController != null) {
                        walletController.convertPoint(point).then((value) {
                          if(value.statusCode == 200){
                            Get.back();
                            customSnackBar('point_converted_successfully'.tr, isError: false);
