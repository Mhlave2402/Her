...
import 'package:her_user_app/features/trip/domain/models/trip_cancelation_cause_list_model.dart';
import 'package:her_user_app/features/trip/domain/models/trip_model.dart';
...
class TripController extends GetxController implements GetxService {
  ...
  final SplitPaymentRepository splitPaymentRepository;
  TripController({required this.tripServiceInterface, required this.splitPaymentRepository});
  ...
  Future<void> requestSplitPayment(int tripId, int userId) async {
    Response response = await splitPaymentRepository.requestSplitPayment(tripId, userId);
    if (response.statusCode == 200) {
      // showCustomSnackBar('Split payment request sent successfully', isError: false);
    } else {
  ...
  Future<void> acceptSplitPayment(int splitPaymentId) async {
    Response response = await splitPaymentRepository.acceptSplitPayment(splitPaymentId);
    if (response.statusCode == 200) {
      // showCustomSnackBar('Split payment accepted successfully', isError: false);
    } else {
      // showCustomSnackBar(response.body['message'], isError: true);
    }
    update();
