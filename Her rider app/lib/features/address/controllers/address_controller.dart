...
class AddressController extends GetxController implements GetxService {
  ...
  Future<void> addNewAddress(Address address, {bool updateAddress = false}) async {
    ...
    if(response.statusCode == 200) {
      ...
      customSnackBar(updateAddress ?'address_updated_successfully'.tr : 'address_added_successfully'.tr, isError: false);
    }else{
  ...
  Future<void> deleteAddress(String addressId) async {
    ...
    if(response!.statusCode == 200){
      ...
      isLoading = false;
      getAddressList(1);
      customSnackBar('address_deleted_successfully'.tr, isError: false);
    }else{
      ApiChecker.checkApi(response);
    }
    isLoading = false;
