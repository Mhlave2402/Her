...
class PaymentController extends GetxController implements GetxService{
  ...
  Future<Response> paymentSubmit(String tripId, String paymentMethod, {bool fromParcel = false} ) async {
    ...
    double walletBalance = Get.find<ProfileController>().profileModel!.data!.walletBalance!;
    double tripAmount = Get.find<RideController>().tripDetails!.trip!.estimatedFare!;

    if (walletBalance >= tripAmount) {
      Response response = await paymentServiceInterface.paymentSubmit(tripId, 'wallet');
      if (response.statusCode == 200 ) {
        Get.find<RideController>().clearRideDetails();
        Get.find<ProfileController>().getProfileInfo();
        customSnackBar('payment_successful'.tr, isError: false);
        NotificationHelper.showTextNotification(
          'Payment Successful',
          'Your wallet balance has been used to pay for the trip.',
          null,
