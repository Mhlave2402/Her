...
import 'package:her_user_app/features/map/screens/map_screen.dart';
import 'package:her_user_app/features/parcel/domain/models/parcel_estimated_fare_model.dart';
import 'package:her_user_app/features/refund_request/controllers/refund_request_controller.dart';
import 'package:her_user_app/features/ride/domain/models/bidding_model.dart';
import 'package:her_user_app/features/ride/domain/models/estimated_fare_model.dart';
import 'package:her_user_app/features/ride/domain/models/final_fare_model.dart';
import 'package:her_user_app/features/ride/domain/models/nearest_driver_model.dart';
import 'package:her_user_app/features/ride/domain/models/remaining_distance_model.dart';
import 'package:her_user_app/features/ride/domain/models/ride_list_model.dart';
...
import 'package:her_user_app/features/ride/screens/driver_found_screen.dart';
import 'package:her_user_app/features/ride/screens/finding_driver_screen.dart';
import 'package:her_user_app/features/trip/controllers/trip_controller.dart';
import 'package:her_user_app/helper/display_helper.dart';
import 'package:her_user_app/helper/pusher_helper.dart';
import 'package:her_user_app/common_widgets/custom_snackbar.dart';
import 'package:her_user_app/helper/ride_controller_helper.dart';
...
enum RideState{initial, riseFare, findingRider, driverFoundPendingAcceptance, outForPickup, afterAcceptRider, otpSent, ongoingRide, completeRide}
enum RideType{regularRide, scheduleRide}
enum DriverGender{female, gay, any}
...
class RideController extends GetxController implements GetxService {
  ...
  List<PremiumFeature> premiumFeatureList = [];
  List<PremiumFeature> selectedPremiumFeatures = [];
  ...
  bool isKidsOnly = false;
  bool isNannyRide = false;
  bool withMaleCompanion = false;
  DriverGender _driverGender = DriverGender.any;
  DriverGender get driverGender => _driverGender;
  bool needBabySeat = false;
  TripDetails? shortfallTrip;
  double _shortfallInstallment = 0;
  ...
  void updateRideCurrentState(RideState newState) {
    ...
    if (newState == RideState.findingRider || newState == RideState.driverFoundPendingAcceptance) {
      Get.to(() => const FindingDriverScreen());
    } else if (newState == RideState.afterAcceptRider) {
      Get.to(() => const DriverFoundScreen());
    }
    update();
  }
  ...
  void toggleKidsOnly() {
    isKidsOnly = !isKidsOnly;
    getEstimatedFare(false);
    update();
  }

  void toggleNannyRide() {
    isNannyRide = !isNannyRide;
    getEstimatedFare(false);
    update();
  }

  void toggleWithMaleCompanion() {
    withMaleCompanion = !withMaleCompanion;
    update();
  }
  ...
  List<String> getSelectedFeatures() {
    ...
    if (isChildFriendly) {
      selectedFeatures.add('child_friendly_mode');
    }
    if (isNannyRide) {
      selectedFeatures.add('nanny_ride');
    }
    if (isKidsOnly) {
      selectedFeatures.add('kids_only_verified_rides');
    }
    if (isBabySeat) {
      selectedFeatures.add('baby_seat_option');
    }
    if (withMaleCompanion) {
      selectedFeatures.add('travel_with_male_companion');
    }
  ...
  void setBabySeat(bool value) {
    needBabySeat = value;
    update();
  }

  void updateDriverGender(DriverGender gender){
  ...
  void setRideCategoryIndex(int newIndex){
    ...
    if(fareList.isNotEmpty){
      ...
      //
      estimatedFare = ((selectedType?.extraFareFee ?? 0) > 0 ? selectedType?.extraEstimatedFare ?? 0 : selectedType?.estimatedFare ?? 0) + _shortfallInstallment;
      currentFarePrice = estimatedFare;
      actualFare = estimatedFare;
      isCouponApplicable = selectedType!.couponApplicable!;
      discountFare = (selectedType?.extraFareFee ?? 0) > 0 ? selectedType?.extraDiscountFare ?? 0 : selectedType?.discountFare ?? 0;
      discountAmount = (selectedType?.extraFareFee ?? 0) > 0 ? selectedType?.extraDiscountAmount ?? 0 : selectedType?.discountAmount ?? 0;
  ...
  Future<Response> getEstimatedFare(bool parcel) async {
    ...
    Response? response = await rideServiceInterface.getEstimatedFare(
      ...
      babySeat: isBabySeat,
      premiumFeatures: selectedPremiumFeatures.map((e) => int.parse(e.id)).toList(),
    );
    ...
    if(response!.statusCode == 200) {
      ...
      if(parcel) {
        ...
        if (shortfallTrip != null && shortfallTrip!.shortfallAmount! > 0) {
          _shortfallInstallment = shortfallTrip!.shortfallAmount! / 5;
        } else {
          _shortfallInstallment = 0;
        }
        fareList = [];
        ...
        if(fareList[rideCategoryIndex].polyline == null){
          customSnackBar('road_not_found'.tr,isError: true);
        }
  ...
  Future<Response> submitRideRequest(String note, bool parcel, {String categoryId = ''}) async {
    ...
    if(response.statusCode == 200 && response.body['data'] != null) {
      ...
      if(tripDetails!.currentStatus == 'driver_found_pending_acceptance'){
        updateRideCurrentState(RideState.driverFoundPendingAcceptance);
      }
  ...
  Future<Response> tripAcceptOrRejected(String tripId, String type, String driverId) async {
    ...
    if (response.statusCode == 200) {
      biddingList = [];
      customSnackBar('trip_is_accepted'.tr, isError: false);
      Get.back();
  ...
  Future<Response> tripStatusUpdate(String tripId, String status, String message, String cancellationCause, {bool afterAccept = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      customSnackBar(message.tr, isError: false);
      isLoading = false;
    }else{
      isLoading = false;
      ApiChecker.checkApi(response);
  ...
  void togglePremiumFeature(PremiumFeature feature) {
    if (selectedPremiumFeatures.contains(feature)) {
      selectedPremiumFeatures.remove(feature);
    } else {
      selectedPremiumFeatures.add(feature);
    }
    getEstimatedFare(false);
    update();
  }

  Future<void> getPremiumFeatures() async {
    Response response = await rideServiceInterface.getPremiumFeatures();
    if (response.statusCode == 200) {
      premiumFeatureList = [];
      response.body.forEach((feature) => premiumFeatureList.add(PremiumFeature.fromJson(feature)));
    } else {
      ApiChecker.checkApi(response);
    }
    update();
  ...
  Future<void> getShortfallTrip() async {
    isLoading = true;
    update();
    Response response = await rideServiceInterface.getShortfallTrip();
    if (response.statusCode == 200 && response.body['data'] != null) {
      shortfallTrip = TripDetailsModel.fromJson(response.body).data!;
    } else {
      shortfallTrip = null;
    }
    isLoading = false;
  ...
  Future<void> payShortfall() async {
    isLoading = true;
    update();
    Response response = await rideServiceInterface.payShortfall(shortfallTrip!.id!);
    if (response.statusCode == 200) {
      shortfallTrip = null;
      customSnackBar('shortfall_paid_successfully'.tr, isError: false);
    } else {
      ApiChecker.checkApi(response);
    }
    isLoading = false;
  ...
  Future<void> reduceShortfall() async {
    if (shortfallTrip != null && shortfallTrip!.shortfallAmount! > 0) {
      final newShortfall = shortfallTrip!.shortfallAmount! - _shortfallInstallment;
      shortfallTrip!.shortfallAmount = newShortfall > 0 ? newShortfall : 0;
      if (newShortfall <= 0) {
        shortfallTrip = null;
      }
      update();
