...
import 'package:her_user_app/features/auth/screens/otp_log_in_screen.dart';
import 'package:her_user_app/features/auth/screens/forgot_password_screen.dart';
...
import 'package:her_user_app/features/auth/screens/sign_up_screen.dart';
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ButtonWidget(
              ...
              onPressed: () {
                ...
                if(phone.isEmpty){
                  customSnackBar('phone_number_is_required'.tr);
                  FocusScope.of(context).requestFocus(phoneNode);
                }else if(!GetUtils.isPhoneNumber(authController.countryDialCode + phone)) {
                  customSnackBar('phone_number_is_not_valid'.tr);
                  FocusScope.of(context).requestFocus(phoneNode);
                }else if(password.isEmpty) {
                  customSnackBar('password_is_required'.tr);
                  FocusScope.of(context).requestFocus(passwordNode);
                }else if(password.length < 8) {
                  customSnackBar('minimum_password_length_is_8'.tr);
                }else {
            ...
            Row(
              ...
              if (Get.find<AuthController>().authServiceInterface.isBiometricAuthEnabled())
                const SizedBox(width: Dimensions.paddingSizeSmall),
              if (Get.find<AuthController>().authServiceInterface.isBiometricAuthEnabled())
                Expanded(
                  child: ButtonWidget(
                        ...
                        TextSpan(
                            ...
                            recognizer: TapGestureRecognizer()..onTap = () async{
                              if(GetPlatform.isAndroid && Get.find<ConfigController>().config?.martPlayStoreUrl != null){
                                ...
                                customSnackBar('contact_with_support'.tr);
                              }
                            }
                        )
                      ]
      ...
      if (availableBiometrics.isNotEmpty) {
        try {
          final bool didAuthenticate = await auth.authenticate(
            localizedReason: 'please_authenticate_to_log_in'.tr,
          );
          if (didAuthenticate) {
            String? faceIdKey = Get.find<AuthController>().getFaceIdKey();
            if (faceIdKey != null && faceIdKey.isNotEmpty) {
              Get.find<AuthController>().biometricLogin(faceIdKey);
            } else {
              customSnackBar('no_face_id_key_found'.tr);
            }
          }
        } catch (e) {
          customSnackBar('authentication_error'.tr);
        }
      }
    } else {
      customSnackBar('biometric_not_supported'.tr);
    }
  }
}
