...
import 'package:her_user_app/features/auth/domain/models/sign_up_body.dart';
...
import 'package:her_user_app/features/auth/screens/reset_password_screen.dart';
import 'package:her_user_app/features/auth/screens/verification_screen.dart';
import 'package:her_user_app/features/parcel/controllers/parcel_controller.dart';
import 'package:her_user_app/features/profile/screens/edit_profile_screen.dart';
...
import 'package:her_user_app/helper/country_code_helper.dart';
...
class AuthController extends GetxController implements GetxService {
    ...
    if (response?.statusCode == 200) {
      ...
      if (isPhoneNotVerified) {
        if (Get.find<ConfigController>().config?.isFirebaseOtpVerification ?? false) {
          ...
          Get.to(() => VerificationScreen(number: fullPhoneNumber, fromOtpLogin: true));
        }else{
          customSnackBar('sms_gateway_not_integrate'.tr);
        }
      }
  ...
  Future<void> externalLogin(String countryCode, String phone, String password) async {
    _isLoading = true;
    update();
    Response? response = await authServiceInterface.externalLogin(phone: countryCode+phone, password: password);
    if(response!.statusCode == 200){
      saveUserNumberAndPassword(countryCode,phone,password,true);
      _isLoading = false;
      setUserToken(response.body['data']['token']);
  ...
  Future<void> logOut() async {
    ...
    if(response!.statusCode == 200){
      ...
      customSnackBar('successfully_logout'.tr, isError: false);
      clearSharedData();
  ...
  Future<Response> sendOtp(String phone) async{
    ...
    if(response!.statusCode == 200){
      _isOtpSending = false;
      customSnackBar('otp_sent_successfully'.tr, isError: false);
    }else{
      _isOtpSending = false;
      ApiChecker.checkApi(response);
    }
      ...
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationFailed: (FirebaseAuthException e) {
          ...
          if(e.code == 'invalid-phone-number') {
            customSnackBar('please_submit_a_valid_phone_number'.tr);
          }else{
            customSnackBar(e.message?.replaceAll('_', ' ') ?? '');
          }
  ...
  Future<Response> otpVerification(String phone, String otp, {bool accountVerification = false,String? session}) async{
    otpVerifying = true;
    update();
    Response? response;
    if(Get.find<ConfigController>().config?.isFirebaseOtpVerification ?? false){
  ...
  Future<Response> otpLogin(String phone, String otp, {bool fromOtpLogin = false}) async{
    ...
    if(response!.statusCode == 200){
      ...
      saveUserNumberAndPassword(countryCode,phoneWithoutCountryCode,Get.find<AuthController>().getUserToken(),true);
      if(fromOtpLogin) {
        _showBiometricOptInDialog();
        Get.find<BottomMenuController>().navigateToDashboard();
      }else{
  ...
  Future<void> biometricLogin(String faceIdKey) async {
    _isLoading = true;
    update();
    Response? response = await authServiceInterface.biometricLogin(faceIdKey: faceIdKey);
    _isLoading = false;
    if (response?.statusCode == 200) {
      setUserToken(response!.body['data']['token']);
      PusherHelper.initializePusher();
  ...
  void saveFaceIdKey(String key) {
    authServiceInterface.saveFaceIdKey(key);
  }

  String? getFaceIdKey() {
    return authServiceInterface.getFaceIdKey();
  }
  ...
  Future<void> permanentlyDelete() async {
    ...
    if(response!.statusCode == 200){
      Get.back();
      Get.offAll(const SignInScreen());
      customSnackBar('successfully_delete_account'.tr, isError: false);
      clearSharedData();
    }else{
      ApiChecker.checkApi(response);
    }
  ...
  void _showBiometricOptInDialog() {
    Get.dialog(
      AlertDialog(
        title: Text('enable_biometric_login'.tr),
        content: Text('do_you_want_to_enable_biometric_login_for_future_access'.tr),
        actions: [
          TextButton(
            onPressed: () => Get.back(),
            child: Text('no'.tr),
          ...
          TextButton(
            onPressed: () {
              authServiceInterface.saveBiometricAuth(true);
              authServiceInterface.saveFaceIdKey(getUserToken());
              Get.back();
            },
            child: Text('yes'.tr),
          ),
