...
import 'package:her_user_app/features/refund_request/domain/models/refund_reason_model.dart';
import 'package:her_user_app/features/refund_request/domain/services/refund_request_service_interface.dart';
import 'package:her_user_app/features/refund_request/widgets/refund_request_send_success_bottomsheet.dart';
    ...
    if(isVideo){
      ...
      if(selectedImage != null){
        if((File(selectedImage.path).lengthSync()/(1024*1024)) > 10){
          customSnackBar('video_length_maximum'.tr);
        }else{
      ...
      if(selectedImage != null){
        if((File(selectedImage.path).lengthSync()/(1024*1024)) > 2){
          customSnackBar('image_length_maximum'.tr);
        }else{
          thumbnailPaths.add('');
          proofImages.add(selectedImage);
          multipartList.add(MultipartBody('attachments[]', selectedImage));
